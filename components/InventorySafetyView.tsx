...
export const InventorySafetyView: React.FC<Props> = ({ data, onItemClick }) => {
  ...
  const warehouses = useMemo(() => ['All', ...Array.from(new Set(data.map(i => i.warehouse)))], [data]);
  const categories = useMemo(() => ['All', ...Array.from(new Set(data.map(i => i.category)))], [data]);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-surface-dark p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 flex flex-wrap gap-4 items-center">
          <div className="flex items-center gap-2">
              ...
              >
                  {warehouses.map(wh => <option key={wh} value={wh}>{wh === 'All' ? '전체 창고' : wh}</option>)}
              </select>
          </div>
          <div className="flex items-center gap-2">
          ...
          <div className="ml-auto text-xs text-gray-500">
              총 {filteredData.length}개 항목 조회됨
          </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-surface-dark rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-5">
            <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4">안전재고 vs 현재재고 현황</h3>
            <div className="h-64">
                    ...
                    >
                        ...
                        <Bar dataKey="currentStock" name="현재재고" fill="#2F5E3E">
                             {filteredData.map((entry, index) => (
                                <Cell key={`cell-${index}`} fill={entry.currentStock < entry.safetyStock ? '#EF4444' : (entry.currentStock > entry.safetyStock * 2 ? '#F59E0B' : '#2F5E3E')} />
                            ))}
                        </Bar>
                    </BarChart>
                </ResponsiveContainer>
            ...
            <div className="mt-2 text-xs text-gray-500 text-center flex justify-center gap-4">
                <span className="flex items-center"><span className="w-2 h-2 rounded-full bg-red-500 mr-1"></span> 부족 (위험)</span>
                <span className="flex items-center"><span className="w-2 h-2 rounded-full bg-green-800 mr-1"></span> 정상</span>
                <span className="flex items-center"><span className="w-2 h-2 rounded-full bg-yellow-500 mr-1"></span> 과잉 (비효율)</span>
            </div>
        </div>
        ...
        <div className="bg-white dark:bg-surface-dark rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-5">
             <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-bold text-gray-900 dark:text-white">재고 회전율 추이 (Turnover Trend)</h3>
             ...
             <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                        ...
                        <Line type="monotone" dataKey="rate" name="회전율" stroke="#3B82F6" strokeWidth={3} dot={{r: 4}} />
                    </LineChart>
                </ResponsiveContainer>
             </div>
      ...
      <div className="bg-white dark:bg-surface-dark rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
        ...
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead className="bg-gray-50 dark:bg-gray-800">
                <tr>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">SKU명</th>
                    <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">창고/카테고리</th>
                    <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">상태</th>
                    <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">현재재고</th>
                    <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">안전재고</th>
                    <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">괴리율</th>
                </tr>
            </thead>
            <tbody className="bg-white dark:bg-surface-dark divide-y divide-gray-200 dark:divide-gray-700">
                {filteredData.filter(i => i.status !== 'Normal').length === 0 ? (
                     <tr><td colSpan={6} className="px-6 py-4 text-center text-sm text-gray-500">리스크 항목이 없습니다.</td></tr>
                ) : (
                    filteredData.filter(i => i.status !== 'Normal').map((item) => (
                    <tr 
                        key={item.id} 
                        className="hover:bg-gray-50 dark:hover:bg-gray-700/50 cursor-pointer"
                        onClick={() => onItemClick && onItemClick(item)}
                    >
                        <td className="px-6 py-4 text-sm font-medium text-gray-900 dark:text-white">{item.skuName}</td>
                        ...
                        <td className="px-6 py-4 text-sm text-right text-gray-900 dark:text-white font-bold">{item.currentStock}</td>
                        <td className="px-6 py-4 text-sm text-right text-gray-500 dark:text-gray-400">{item.safetyStock}</td>
                        <td className="px-6 py-4 text-sm text-right text-gray-500 dark:text-gray-400">
                            {Math.round(((item.currentStock - item.safetyStock) / item.safetyStock) * 100)}%
                        </td>
                    </tr>
                )))}
            </tbody>
