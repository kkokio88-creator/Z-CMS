...
import { fetchChannelProfitability, formatCurrency, getMarginColorClass } from '../services/costAnalysisService';
...
const NoDataState = ({ title, description }: { title: string; description: string }) => (
    <div className="flex flex-col items-center justify-center h-96 bg-white dark:bg-surface-dark rounded-lg border border-gray-200 dark:border-gray-700">
        <span className="material-icons-outlined text-6xl text-gray-300 dark:text-gray-600 mb-4">cloud_off</span>
        ...
        <div className="mt-6 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800 max-w-md">
            <p className="text-sm text-blue-700 dark:text-blue-300">
                ...
                판매 데이터를 보려면 ECOUNT에서 판매 조회 API를 구독하거나 수동으로 데이터를 입력해야 합니다.
            </p>
        </div>
    </div>
);
...
export const ChannelProfitView: React.FC<Props> = ({ data }) => {
    ...
    const cogsChartData = selectedChannelData ? [
        { name: '원재료비', value: selectedChannelData.cogs.rawMaterial, color: COGS_COLORS.rawMaterial },
        ...
        { name: '수수료', value: selectedChannelData.cogs.commission, color: COGS_COLORS.commission },
    ...
    const renderCOGSTab = () => (
        <>
            {isLoading ? (
                ...
                <NoDataState title="COGS 데이터 없음" description="채널별 원가 데이터를 불러올 수 없습니다." />
            ) : (
                <>
                    {/* Channel Selector */}
                    <div className="bg-white dark:bg-surface-dark rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4">
                        <div className="flex items-center gap-4 flex-wrap">
                            <span className="text-sm font-medium text-gray-700 dark:text-gray-300">채널 선택:</span>
                        ...
                        <>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                <div className="bg-white dark:bg-surface-dark rounded-lg p-4 border border-gray-200 dark:border-gray-700">
                                    ...
                                    <p className="text-xl font-bold text-gray-900 dark:text-white">₩{formatCurrency(selectedChannelData.revenue)}</p>
                                </div>
                                <div className="bg-white dark:bg-surface-dark rounded-lg p-4 border border-gray-200 dark:border-gray-700">
                                    <p className="text-xs text-gray-500 dark:text-gray-400">총 원가 (COGS)</p>
                                    <p className="text-xl font-bold text-red-600 dark:text-red-400">₩{formatCurrency(selectedChannelData.totalCogs)}</p>
                                </div>
                                <div className="bg-white dark:bg-surface-dark rounded-lg p-4 border border-gray-200 dark:border-gray-700">
                                    <p className="text-xs text-gray-500 dark:text-gray-400">매출총이익</p>
                                    <p className="text-xl font-bold text-green-600 dark:text-green-400">₩{formatCurrency(selectedChannelData.grossProfit)}</p>
                                </div>
                                <div className="bg-white dark:bg-surface-dark rounded-lg p-4 border border-gray-200 dark:border-gray-700">
                                    <p className="text-xs text-gray-500 dark:text-gray-400">매출총이익률</p>
                                    <p className={`text-xl font-bold ${selectedChannelData.grossMargin >= 20 ? 'text-green-600 dark:text-green-400' : selectedChannelData.grossMargin >= 10 ? 'text-yellow-600 dark:text-yellow-400' : 'text-red-600 dark:text-red-400'}`}>
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                {/* Pie Chart */}
                                <div className="bg-white dark:bg-surface-dark rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                                    <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4">원가 구성 (COGS Breakdown)</h3>
                                    <div className="h-64">
                                        <ResponsiveContainer width="100%" height="100%">
                                            <PieChart>
                                                <Pie
                                ...
                                {/* COGS Detail Table */}
                                <div className="bg-white dark:bg-surface-dark rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                                    <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4">원가 항목별 상세</h3>
                            ...
                            <div className="bg-white dark:bg-surface-dark rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                        <span className={`material-icons-outlined ${selectedChannelData.profitTrend === 'up' ? 'text-green-500' : selectedChannelData.profitTrend === 'down' ? 'text-red-500' : 'text-gray-500'}`}>
                                            {selectedChannelData.profitTrend === 'up' ? 'trending_up' : selectedChannelData.profitTrend === 'down' ? 'trending_down' : 'trending_flat'}
                                        </span>
                                        <span className="text-sm text-gray-700 dark:text-gray-300">
                                            전월 대비 {selectedChannelData.profitTrend === 'up' ? '개선' : selectedChannelData.profitTrend === 'down' ? '악화' : '유지'}
                                        </span>
                                    </div>
                                    <span className={`text-sm font-bold ${selectedChannelData.trendPercent >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                                        {selectedChannelData.trendPercent >= 0 ? '+' : ''}{selectedChannelData.trendPercent.toFixed(1)}%
                                    </span>
                                </div>
                            </div>
                        </>
    ...
    const renderContributionTab = () => (
        <>
            {isLoading ? (
                ...
                <NoDataState title="공헌이익 데이터 없음" description="채널별 공헌이익 데이터를 불러올 수 없습니다." />
            ) : (
                <>
                    {/* Margin Comparison Bar Chart */}
                    <div className="bg-white dark:bg-surface-dark rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                        <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4">채널별 이익률 비교</h3>
                        <div className="h-80">
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={contributionData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                                    ...
                                    <Legend />
                                    <Bar dataKey="grossMargin" name="매출총이익률" fill="#3B82F6" radius={[4, 4, 0, 0]} />
                                    <Bar dataKey="contributionMargin" name="공헌이익률" fill="#10B981" radius={[4, 4, 0, 0]} />
                    ...
                    <div className="bg-white dark:bg-surface-dark rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                        ...
                        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            <thead className="bg-gray-50 dark:bg-gray-800">
                                <tr>
                                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">채널</th>
                                    <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">매출</th>
                                    <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">매출총이익률</th>
                                    <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">공헌이익률</th>
                                    <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">순이익률</th>
                                    <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">추세</th>
                                </tr>
                            </thead>
                            <tbody className="bg-white dark:bg-surface-dark divide-y divide-gray-200 dark:divide-gray-700">
                                {channelProfitability.map((channel, idx) => (
                                    <tr key={idx} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                        <td className="px-6 py-4">
                                            <div>
                                                <p className="text-sm font-medium text-gray-900 dark:text-white">{channel.channelName}</p>
                                                <p className="text-xs text-gray-500 dark:text-gray-400">{channel.channelType}</p>
                                            </div>
                                        </td>
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <span className={`inline-block px-2 py-1 rounded text-xs font-bold ${getMarginColorClass(channel.grossMargin)}`}>
                                                {channel.grossMargin.toFixed(1)}%
                                            </span>
                                        </td>
                                        <td className="px-6 py-4 text-right">
                                            <span className={`inline-block px-2 py-1 rounded text-xs font-bold ${getMarginColorClass(channel.contributionMargin)}`}>
                                                {channel.contributionMargin.toFixed(1)}%
                                            </span>
                                        </td>
                                        <td className="px-6 py-4 text-right">
                                            <span className={`inline-block px-2 py-1 rounded text-xs font-bold ${getMarginColorClass(channel.netMargin)}`}>
                                                {channel.netMargin.toFixed(1)}%
                                            </span>
                                        </td>
                                        <td className="px-6 py-4 text-center">
                                            <span className={`material-icons-outlined text-lg ${
                                                channel.profitTrend === 'up' ? 'text-green-500' :
                                                channel.profitTrend === 'down' ? 'text-red-500' : 'text-gray-400'
                                            }`}>
                                                {channel.profitTrend === 'up' ? 'arrow_upward' :
                                                 channel.profitTrend === 'down' ? 'arrow_downward' : 'remove'}
                                            </span>
                                        </td>
                                    </tr>
                                ))}
                                    ...
                                    <div className="flex items-start gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-orange-600 dark:text-orange-300 mt-1">
                                                공헌이익률 {channel.contributionMargin.toFixed(1)}% - 수수료 비중({((channel.cogs.commission / channel.totalCogs) * 100).toFixed(0)}%) 검토 권장
                                            </p>
                                        </div>
                                    </div>
                                </div>
        ...
        <div className="space-y-6">
            ...
            <div className="border-b border-gray-200 dark:border-gray-700">
                <div className="flex gap-2">
                    ...
                    <TabButton active={activeTab === 'contribution'} onClick={() => setActiveTab('contribution')}>
                        공헌이익률 분석
                    </TabButton>
                </div>
            </div>
