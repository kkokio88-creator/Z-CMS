      ...
      <div className="relative">
        ...
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          <thead className="bg-gray-50 dark:bg-gray-800">
            <tr>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">일시</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">대상 품목</th>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">변경 전/후</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">작업자</th>
            </tr>
          </thead>
              ...
              <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                   <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${
                       item.actionType === 'Update' ? 'bg-indigo-100 text-indigo-800 dark:bg-indigo-900 dark:text-indigo-200' :
                       item.actionType === 'Fix' ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200' :
                ...
                <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">
                   {item.oldValue && item.newValue ? (
                       <div className="flex items-center gap-2">
                           ...
                           <span className="font-bold text-green-600 dark:text-green-400">{item.newValue}</span>
                       </div>
                   ) : (
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm">
                   <div className="flex items-center gap-2">
                       ...
                       <span className={item.actor === 'AI Agent' ? 'text-purple-600 dark:text-purple-400 font-medium' : 'text-gray-600 dark:text-gray-300'}>
                           {item.actor}
                       </span>
                   </div>
                </td>
