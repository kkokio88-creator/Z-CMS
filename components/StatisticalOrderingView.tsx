...
const StatisticalOrderingView: React.FC = () => {
    const [activeTab, setActiveTab] = useState<'overview' | 'mealplan' | 'stats' | 'config'>('overview');
    ...
    const statusData = recommendation ? [
        { name: '정상', value: recommendation.items.filter(i => i.status === 'normal').length, color: STATUS_COLORS.normal },
        { name: '긴급', value: recommendation.items.filter(i => i.status === 'urgent').length, color: STATUS_COLORS.urgent },
        { name: '부족', value: recommendation.items.filter(i => i.status === 'shortage').length, color: STATUS_COLORS.shortage },
        { name: '과재고', value: recommendation.items.filter(i => i.status === 'overstock').length, color: STATUS_COLORS.overstock },
    ].filter(d => d.value > 0) : [];
                    ...
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead className="bg-gray-50 dark:bg-gray-800">
                            <tr>
                                ...
                                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">품목명</th>
                                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">분류</th>
                                <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">총소요량</th>
                                <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">안전재고</th>
                                <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">현재고</th>
                                <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">입고예정</th>
                                <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">발주수량</th>
                                <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">예상금액</th>
                            </tr>
                        </thead>
                        <tbody className="bg-white dark:bg-surface-dark divide-y divide-gray-200 dark:divide-gray-700">
                            {filteredItems.slice(0, 20).map((item, idx) => (
                                <tr key={idx} className={`hover:bg-gray-50 dark:hover:bg-gray-700/50 ${
                                    item.status === 'shortage' ? 'bg-red-50/50 dark:bg-red-900/10' :
                                    item.status === 'urgent' ? 'bg-orange-50/50 dark:bg-orange-900/10' : ''
                                }`}>
                                    <td className="px-4 py-3">
                                        <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${getStatusColorClass(item.status)}`}>
                                            <span className="material-icons-outlined text-sm">{getStatusIcon(item.status)}</span>
                                            {getStatusLabel(item.status)}
                                        </span>
                                    </td>
                                    <td className="px-4 py-3">
                                        <div>
                                            <p className="text-sm font-medium text-gray-900 dark:text-white">{item.ingredientName}</p>
                ...
                <div className="bg-white dark:bg-surface-dark rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                    <div className="flex items-center justify-between mb-4">
                        <h3 className="text-lg font-bold text-gray-900 dark:text-white">
                            미래 식단 계획 (D+{recommendation?.leadTimeDays || 2} ~ D+{(recommendation?.leadTimeDays || 2) + 7})
                        </h3>
                                    ...
                                    <div className="space-y-2">
                                        {meals.map((meal, idx) => (
                                            <div key={idx} className="flex items-center justify-between text-sm">
                                                ...
                                                <span className="text-gray-900 dark:text-white truncate ml-2">{meal.menuName}</span>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                        ...
                        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            <thead className="bg-gray-50 dark:bg-gray-800 sticky top-0">
                                <tr>
                                    ...
                                    <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">평균</th>
                                    <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">표준편차</th>
                                    <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">최소</th>
                                    <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">최대</th>
                                    <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">샘플수</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                                {salesStats.slice(0, 50).map((stat, idx) => (
                                    <tr key={idx} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                        <td className="px-4 py-2 text-sm text-gray-900 dark:text-white">{stat.dayOfWeek}</td>
                                        <td className="px-4 py-2 text-sm text-gray-900 dark:text-white">{stat.menuName}</td>
                                        <td className="px-4 py-2 text-sm text-right font-medium text-gray-900 dark:text-white">{stat.avgSales}</td>
                                        <td className="px-4 py-2 text-sm text-right text-gray-600 dark:text-gray-400">±{stat.stdDev}</td>
                                        <td className="px-4 py-2 text-sm text-right text-gray-600 dark:text-gray-400">{stat.minSales}</td>
                                        <td className="px-4 py-2 text-sm text-right text-gray-600 dark:text-gray-400">{stat.maxSales}</td>
                                        <td className="px-4 py-2 text-sm text-right text-gray-500">{stat.sampleCount}주</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
            ...
            <div className="bg-white dark:bg-surface-dark rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                ...
                <div className="mt-8 p-4 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
                    ...
                    <ul className="mt-3 text-sm text-gray-600 dark:text-gray-400 space-y-1">
                        <li>• <strong>Z</strong>: 서비스 계수 (현재 {config?.zScore || 1.65})</li>
                        <li>• <strong>σ</strong>: 수요의 표준편차 (요일별 판매량 변동)</li>
                        <li>• <strong>L</strong>: 리드타임 + 안전일수 ({(config?.defaultLeadTime || 2) + (config?.safetyDays || 1)}일)</li>
                    </ul>
                </div>
            </div>
        </div>
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                ...
                >
                    <span className={`material-icons-outlined ${isLoading ? 'animate-spin' : ''}`}>refresh</span>
                    새로고침
                </button>
            </div>
            ...
            <div className="border-b border-gray-200 dark:border-gray-700">
                <div className="flex gap-2 overflow-x-auto">
                    <TabButton active={activeTab === 'overview'} onClick={() => setActiveTab('overview')} icon="dashboard">
                        발주 개요
                    </TabButton>
                    <TabButton active={activeTab === 'mealplan'} onClick={() => setActiveTab('mealplan')} icon="restaurant_menu">
                        식단 계획
                    </TabButton>
                    <TabButton active={activeTab === 'stats'} onClick={() => setActiveTab('stats')} icon="analytics">
                        판매 통계
                    </TabButton>
