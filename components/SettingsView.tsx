...
export const SettingsView: React.FC = () => {
    ...
    // 데이터 소스 연결 관리 State
    const [dataSourcesConfig, setDataSourcesConfig] = useState<DataSourcesConfig>(defaultDataSourcesConfig);
    ...
    const renderStatusBadge = (status: DataSourceConfig['status']) => {
        ...
        return (
            <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${styles[status]}`}>
                <span className={`material-icons-outlined text-sm mr-1 ${status === 'testing' ? 'animate-spin' : ''}`}>
                    {icons[status]}
                </span>
                {labels[status]}
            </span>
            ...
            <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center">
                        <span className="material-icons-outlined text-gray-500 dark:text-gray-400 mr-2">{icon}</span>
                        <div>
                ...
                {/* 데이터 소스 타입 선택 */}
                <div className="mb-3">
                    <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">연결 방식</label>
                    ...
                    <div className="space-y-3 pt-3 border-t border-gray-100 dark:border-gray-700">
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <button
                                ...
                                disabled={isTesting || !config.googleSheets?.spreadsheetUrl || !config.googleSheets?.sheetName}
