...
export const fetchMaterialPriceHistory = async (
    ...
    try {
        const response = await fetch(`${BACKEND_URL}/cost-analysis/material-price-trend?weeks=${weeks}`);
        const result = await response.json();
...
export const fetchChannelProfitability = async (
    ...
    try {
        const response = await fetch(`${BACKEND_URL}/cost-analysis/channel-profitability?period=${period}`);
        const result = await response.json();
 ...
 * Anomaly Level 결정
 */
export const determineAnomalyLevel = (gap: number, thresholds = { warning: 5, critical: 10 }): AnomalyLevel => {
 ...
 * Budget Status 결정
 */
export const determineBudgetStatus = (burnRate: number, daysElapsed: number, daysInMonth: number): BudgetStatus => {
 ...
 */
export const determinePerformanceStatus = (actual: number, target: number, tolerance = 5): PerformanceStatus => {
