...
function transformBom(raw: Record<string, any>[], brand: 'SAN' | 'ZIP'): BomItem[] {
    return raw.map(row => ({
        ...
        ingredientName: String(row['원재료명'] || row['자재명'] || row['재료명'] || row['원재료'] || ''),
...
export async function fetchAllIntegratedData(): Promise<IntegratedData> {
    ...
    if (config.mealPlan.type === 'googleSheets' && config.mealPlan.googleSheets?.spreadsheetUrl) {
        fetchTasks.push(
            fetchSheetData(config.mealPlan.googleSheets.spreadsheetUrl, config.mealPlan.googleSheets.sheetName)
                .then(res => {
                    if (res.success) {
    ...
    // 판매실적
    if (config.salesHistory.type === 'googleSheets' && config.salesHistory.googleSheets?.spreadsheetUrl) {
        fetchTasks.push(
            fetchSheetData(config.salesHistory.googleSheets.spreadsheetUrl, config.salesHistory.googleSheets.sheetName)
                .then(res => {
                    if (res.success) {
    ...
    if (config.bomSan.type === 'googleSheets' && config.bomSan.googleSheets?.spreadsheetUrl) {
        fetchTasks.push(
            fetchSheetData(config.bomSan.googleSheets.spreadsheetUrl, config.bomSan.googleSheets.sheetName)
                .then(res => {
                    if (res.success) {
    ...
    if (config.bomZip.type === 'googleSheets' && config.bomZip.googleSheets?.spreadsheetUrl) {
        fetchTasks.push(
            fetchSheetData(config.bomZip.googleSheets.spreadsheetUrl, config.bomZip.googleSheets.sheetName)
                .then(res => {
                    if (res.success) {
    ...
    if (config.inventory.type === 'googleSheets' && config.inventory.googleSheets?.spreadsheetUrl) {
        fetchTasks.push(
            fetchSheetData(config.inventory.googleSheets.spreadsheetUrl, config.inventory.googleSheets.sheetName)
                .then(res => {
                    if (res.success) {
    ...
    // 구매현황
    if (config.purchaseHistory.type === 'googleSheets' && config.purchaseHistory.googleSheets?.spreadsheetUrl) {
        fetchTasks.push(
            fetchSheetData(config.purchaseHistory.googleSheets.spreadsheetUrl, config.purchaseHistory.googleSheets.sheetName)
                .then(res => {
                    if (res.success) {
                        result.purchaseHistory = transformPurchaseHistory(res.data);
                        result.sourceStatus.purchaseHistory = { loaded: true, rowCount: result.purchaseHistory.length };
                    } else {
    ...
    // 발주현황
    if (config.purchaseOrders.type === 'googleSheets' && config.purchaseOrders.googleSheets?.spreadsheetUrl) {
        fetchTasks.push(
            fetchSheetData(config.purchaseOrders.googleSheets.spreadsheetUrl, config.purchaseOrders.googleSheets.sheetName)
                .then(res => {
                    if (res.success) {
                        result.purchaseOrders = transformPurchaseOrders(res.data);
                        result.sourceStatus.purchaseOrders = { loaded: true, rowCount: result.purchaseOrders.length };
                    } else {
...
export function calculateMenuCost(
    ...
    bom: BomItem[],
    purchaseHistory: PurchaseRecord[]
): { totalCost: number; ingredients: { name: string; qty: number; unitCost: number; cost: number }[] } {
...
// 메뉴별 판매량 집계
export function aggregateSalesByMenu(sales: SalesRecord[]): Map<string, { qty: number; amount: number }> {
...
// 일별 판매 트렌드
export function getDailySalesTrend(sales: SalesRecord[]): { date: string; amount: number; qty: number }[] {
