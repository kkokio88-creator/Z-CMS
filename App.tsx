...
import { AgentProvider } from './src/agents/AgentContext.tsx';
...
import {
    Notification, ChannelProfitData, InventorySafetyItem, BomDiffItem, DashboardSummary,
    ProfitRankItem, StocktakeAnomalyItem, OrderSuggestion, WasteTrendData,
    BomYieldAnalysisItem, InventoryDiscrepancyItem, MaterialPriceHistory, MaterialCostImpact,
...
const App = () => {
  ...
  const renderActiveView = () => {
    ...
    switch (activeView) {
      ...
      case 'costreport': return <CostReportView />;
      case 'costmgmt': return <CostManagementDashboard />;
      case 'bomaudit': return <BomIntegrityAuditView />;
      case 'priceimpact': return <MaterialPriceImpactView />;
      case 'dailyperformance': return <DailyPerformanceView />;
      case 'budgetexpense': return <BudgetExpenseView />;
      case 'statorder': return <StatisticalOrderingView />;
      default: return (
        <DashboardHomeView
            onSync={handleEcountSync}
            isSyncing={isSyncing}
  ...
  const getPageTitle = () => {
      switch (activeView) {
          ...
          case 'costreport': return '원가 리포트';
          case 'costmgmt': return '종합 원가관리';
          case 'bomaudit': return 'BOM 정합성 검토';
          case 'priceimpact': return '단가 변동 분석';
          case 'dailyperformance': return '일일 달성률';
          case 'budgetexpense': return '예산/경비 관리';
          case 'statorder': return '통계적 발주 관리';
          default: return '대시보드';
      }
  };
  ...
  return (
    <AgentProvider autoConnect={true}>
      <div className="flex h-screen bg-background-light dark:bg-background-dark overflow-hidden font-sans">
        <Sidebar activeView={activeView} onNavigate={setActiveView} dataAvailability={dataAvailability} />
