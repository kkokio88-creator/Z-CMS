...
export class StatisticalOrderingService {
    ...
    constructor(config?: Partial<OrderingConfig>) {
        this.config = {
            ...
            defaultLeadTime: 2,
            safetyDays: 1,
            mealPlanSpreadsheetId: process.env.MEAL_PLAN_SPREADSHEET_ID || '1395EnPHzgOKCZ-kgSYzjNr84_QHPqIkQI8L9IBQUgWE',
            ...config,
        };
    }
    ...
    private async fetchSheetData(spreadsheetId: string, sheetName: string): Promise<Record<string, any>[]> {
        try {
    ...
    async fetchSalesHistory(weeks: number = 4): Promise<SalesHistoryItem[]> {
        ...
        if (rawData.length === 0) {
            ...
            const historyData = await this.fetchSheetData(this.config.mealPlanSpreadsheetId, '식단_히스토리');
            return historyData
                .filter(row => {
    ...
    calculateDayOfWeekStats(salesHistory: SalesHistoryItem[]): Map<string, DayOfWeekStats> {
        ...
        grouped.forEach((sales, key) => {
            ...
            const menuName = salesHistory.find(s => (s.menuCode || s.menuName) === menuCode)?.menuName || menuCode;
    ...
    calculateOrderRecommendation(
        ...
        // 2. BOM 전개 → 식자재별 소요량
        const ingredientRequirement = new Map<string, {
            grossQty: number;
            stdDev: number;
            menuSources: string[];
        }>();
        ...
        menuDemand.forEach((demand, menuKey) => {
            const menuRecipes = recipes.filter(r =>
                r.menuCode === menuKey || r.menuName === menuKey
            );
        ...
        const ingredientMap = new Map(
            ingredients.map(i => [i.ingredientCode || i.ingredientName, i])
        );
        ...
        // 요약 통계
        const urgentItems = orderItems.filter(i => i.status === 'urgent' || i.status === 'shortage').length;
        ...
        console.log(`[StatisticalOrdering] 발주 권고 ${orderItems.length}건 생성 (긴급: ${urgentItems}, 부족: ${shortageItems})`);
