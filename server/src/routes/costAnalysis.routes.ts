...
const determineBudgetStatus = (burnRate: number, daysElapsed: number, daysInMonth: number): BudgetStatus => {
...
function transformInventoryDiscrepancy(inventory: EcountInventoryRaw[], production: EcountProductionRaw[]): any[] {
    ...
    return inventory.map((item, idx) => {
        ...
        const discrepancyRate = transactionQty !== 0 ? (discrepancyQty / Math.abs(transactionQty)) * 100 : 0;
...
function transformMaterialPriceHistory(purchases: EcountPurchaseRaw[], weeks: number = 12): any[] {
    ...
    const priceByMaterial = new Map<string, { prices: { date: string; price: number; supplier: string }[]; name: string }>();
    ...
    priceByMaterial.forEach((data, code) => {
        ...
        const priceChangeWeek = weekAgoPrice > 0 ? ((currentPrice - weekAgoPrice) / weekAgoPrice) * 100 : 0;
        const priceChangeMonth = monthAgoPrice > 0 ? ((currentPrice - monthAgoPrice) / monthAgoPrice) * 100 : 0;
...
function calculateMaterialImpacts(priceHistory: any[], bom: EcountBomRaw[]): any[] {
    ...
    const increasedMaterials = priceHistory.filter(m => m.priceChangeWeek > 5 || m.priceChangeMonth > 10);
    ...
    return increasedMaterials.map(material => {
        ...
        return {
            ...
            affectedProducts,
            totalDeltaCost,
            urgencyLevel: material.priceChangeWeek > 15 ? 'critical' : material.priceChangeWeek > 10 ? 'warning' : 'normal'
        };
    });
}

function transformChannelProfitability(sales: EcountSaleRaw[], purchases: EcountPurchaseRaw[], period: string = '30days'): any[] {
    ...
    const totalPurchaseCost = purchases.reduce((sum, p) => sum + (parseFloat(p.SUPPLY_AMT as any) || 0), 0);
...
function determineChannelType(channelName: string): string {
    ...
    if (name.includes('쿠팡') || name.includes('마켓') || name.includes('11번가')) return 'Marketplace';
...
function transformDailyPerformance(production: EcountProductionRaw[], purchases: EcountPurchaseRaw[]): any[] {
    // Group by date
    ...
    dailyData.forEach((data, date) => {
        ...
        const actualMaterialRatio = estimatedRevenue > 0 ? (data.materialCost / estimatedRevenue) * 100 : 0;
        ...
        result.push({
            ...
            efficiency: Math.round(100 - Math.abs(actualLaborRatio - targetLaborRatio) - Math.abs(actualMaterialRatio - targetMaterialRatio)),
...
function transformBudgetData(purchases: EcountPurchaseRaw[], period?: string): { budgetItems: any[]; expenseSummary: any } {
    // Group purchases by supplier (as budget category proxy)
    const budgetByVendor = new Map<string, number>();

    purchases.forEach(p => {
        const vendor = p.CUST_DES || '기타';
        budgetByVendor.set(vendor, (budgetByVendor.get(vendor) || 0) + (parseFloat(p.SUPPLY_AMT as any) || 0));
    });
    ...
    const expenseSummary = {
        ...
        healthScore: Math.max(0, Math.min(100, 100 - (overallBurnRate - (daysElapsed / daysInMonth) * 100)))
    };
    ...
    return {
        ...
        data: {
            yieldHistory: production.map((p: any) => ({
                ...
                yield: parseFloat(p.QTY) > 0 ? ((parseFloat(p.QTY) - parseFloat(p.USE_QTY)) / parseFloat(p.QTY)) * 100 : 100
            })),
            suggestedActions: [
                'BOM 표준 업데이트',
                '로스 원인 조사 요청'
