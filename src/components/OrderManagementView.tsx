...
export const OrderManagementView: React.FC<Props> = ({ suggestions: initialSuggestions }) => {
  ...
  const [bulkExecutionProgress, setBulkExecutionProgress] = useState(0);

  const handleQuantityChange = (id: string, newQty: number) => {
    setSuggestions(prev =>
      prev.map(item => (item.id === id ? { ...item, orderQty: newQty } : item))
    );
  };
  ...
  // 전체 선택/해제
  const pendingItems = suggestions.filter(item => item.status !== 'Sent');
  const isAllSelected = pendingItems.length > 0 && pendingItems.every(item => selectedItems.has(item.id));
  ...
  const handleBulkOrder = async () => {
    ...
    setBulkExecutionProgress(0);

    const selectedList = suggestions.filter(item => selectedItems.has(item.id) && item.status !== 'Sent');
    const total = selectedList.length;

    for (let i = 0; i < selectedList.length; i++) {
      const item = selectedList[i];
      // 발주 전송 시뮬레이션 (실제로는 API 호출)
      await new Promise(resolve => setTimeout(resolve, 500));

      setSuggestions(prev =>
        prev.map(s => (s.id === item.id ? { ...s, status: 'Sent' } : s))
      );
      setBulkExecutionProgress(Math.round(((i + 1) / total) * 100));
    }

    setSelectedItems(new Set());
    setIsBulkExecuting(false);
    alert(`${total}건의 발주가 완료되었습니다.`);
  };
  ...
  const confirmOrder = () => {
    ...
    setTimeout(() => {
      alert(
        `${executingOrder.supplierName}으로 ${executingOrder.orderQty}${executingOrder.unit} 발주가 ${executingOrder.method}로 전송되었습니다.`
      );
      setSuggestions(prev =>
        prev.map(item => (item.id === executingOrder.id ? { ...item, status: 'Sent' } : item))
      );
    ...
    if (item.method === 'Email') {
      return (
        <div className="border border-gray-300 dark:border-gray-600 rounded-md p-4 bg-gray-50 dark:bg-gray-800 font-mono text-sm">
          ...
          <p className="border-b border-gray-200 dark:border-gray-700 pb-2 mb-2">
            <span className="font-bold text-gray-500">Subject:</span> [발주요청] {item.skuName} 발주
            건 (Z-CMS)
          </p>
          <div className="space-y-2 text-gray-800 dark:text-gray-300">
            ...
            <ul className="list-disc list-inside pl-2 py-2 bg-white dark:bg-gray-700 rounded border border-gray-200 dark:border-gray-600">
              ...
              <li>
                납기요청일: {new Date(Date.now() + item.leadTime * 86400000).toLocaleDateString()}{' '}
                (Lead Time: {item.leadTime}일)
              </li>
        ...
        <div className="max-w-xs mx-auto bg-yellow-100 dark:bg-yellow-900/30 rounded-lg p-4 relative border border-yellow-200 dark:border-yellow-700">
          ...
          <div className="text-sm space-y-1 text-gray-800 dark:text-gray-200">
            ...
            <div className="my-2 border-t border-yellow-200 dark:border-yellow-700 pt-1">
              <p>- 품목: {item.skuName}</p>
              <p>
                - 수량: {item.orderQty} {item.unit}
              </p>
              <p>- 금액: ₩{total}</p>
            </div>
            <p className="text-xs text-gray-500">하단 버튼을 눌러 수락해주세요.</p>
            {item.method === 'Kakao' && (
              <button className="w-full mt-2 bg-yellow-300 hover:bg-yellow-400 text-black text-xs font-bold py-2 rounded">
                발주 확인하기
              </button>
            )}
          </div>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            disabled={selectedItems.size === 0 || isBulkExecuting}
            className="flex items-center px-4 py-2 bg-primary hover:bg-primary-hover text-white rounded-lg text-sm font-medium shadow-sm transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <span className="material-icons-outlined text-sm mr-2">send</span>
            {isBulkExecuting
              ? `발주 중... (${bulkExecutionProgress}%)`
              : `선택 항목 일괄 발주 (${selectedItems.size}건)`}
          </button>
        </div>
      </div>
