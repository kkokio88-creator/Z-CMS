...
export const DataSourceStatus: React.FC<DataSourceStatusProps> = ({
  ...
  const { data, isLoading, error, lastUpdated, connectedSources, refreshData, hasConfiguration } =
    useIntegratedData();
    ...
    return (
      <div className="flex items-center gap-2">
        ...
        {showRefreshButton && (
          ...
          >
            <span
              className={`material-icons-outlined text-sm text-gray-500 ${isLoading ? 'animate-spin' : ''}`}
            >
              refresh
            </span>
          </button>
      ...
      <div className="flex items-center gap-2">
        <span
          className={`material-icons-outlined text-sm ${
            hasError ? 'text-red-500' : isLoaded ? 'text-green-500' : 'text-gray-400'
          }`}
        >
