...
const createMockData = (): MonthlyCostSummary[] => [
  {
    ...
    rawMaterialCost: 18000000,
    subMaterialCost: 5000000,
    laborCost: 8500000,
    expenseAmount: 6500000,
    totalCost: 38000000,
    wasteCost: 2200000,
    profitRatio: 1.37,
    rawMaterialRatio: 2.89,
    subMaterialRatio: 10.4,
    laborRatio: 6.12,
    expenseRatio: 8.0,
    targetSales: 55000000,
    targetRawMaterial: 17000000,
    targetSubMaterial: 4500000,
    targetLabor: 8000000,
    targetExpense: 6000000,
    targetRatio: 1.55,
    achievementRate: 88,
    rawMaterialAchievement: 94,
    subMaterialAchievement: 90,
    laborAchievement: 94,
    expenseAchievement: 92,
    rawMaterialVariance: -1000000,
    subMaterialVariance: -500000,
    laborVariance: -500000,
    expenseVariance: -500000,
  },
  {
    month: '2025-11',
    salesAmount: 58000000,
    rawMaterialCost: 19500000,
    subMaterialCost: 5200000,
    laborCost: 9000000,
    expenseAmount: 6800000,
    totalCost: 40500000,
    wasteCost: 1800000,
    profitRatio: 1.43,
    rawMaterialRatio: 2.97,
    subMaterialRatio: 11.15,
    laborRatio: 6.44,
    expenseRatio: 8.53,
    targetSales: 58000000,
    targetRawMaterial: 19000000,
    targetSubMaterial: 5000000,
    targetLabor: 8800000,
    targetExpense: 6500000,
    targetRatio: 1.47,
    achievementRate: 97,
    rawMaterialAchievement: 97,
    subMaterialAchievement: 96,
    laborAchievement: 98,
    expenseAchievement: 96,
    rawMaterialVariance: -500000,
    subMaterialVariance: -200000,
    laborVariance: -200000,
    expenseVariance: -300000,
  },
  {
    month: '2025-12',
    salesAmount: 63000000,
    rawMaterialCost: 20000000,
    subMaterialCost: 5500000,
    laborCost: 9500000,
    expenseAmount: 7000000,
    totalCost: 42000000,
    wasteCost: 2500000,
    profitRatio: 1.5,
    rawMaterialRatio: 3.15,
    subMaterialRatio: 11.45,
    laborRatio: 6.63,
    expenseRatio: 9.0,
    targetSales: 60000000,
    targetRawMaterial: 20000000,
    targetSubMaterial: 5500000,
    targetLabor: 9500000,
    targetExpense: 7000000,
    targetRatio: 1.43,
    achievementRate: 105,
    rawMaterialAchievement: 100,
    subMaterialAchievement: 100,
    laborAchievement: 100,
    expenseAchievement: 100,
    rawMaterialVariance: 0,
    subMaterialVariance: 0,
    laborVariance: 0,
    expenseVariance: 0,
  },
  {
    ...
    rawMaterialCost: 17000000,
    subMaterialCost: 4800000,
    laborCost: 8200000,
    expenseAmount: 6200000,
    totalCost: 36200000,
    wasteCost: 1900000,
    profitRatio: 1.33,
    rawMaterialRatio: 2.82,
    subMaterialRatio: 10.0,
    laborRatio: 5.85,
    expenseRatio: 7.74,
    targetSales: 50000000,
    targetRawMaterial: 16000000,
    targetSubMaterial: 4500000,
    targetLabor: 8000000,
    targetExpense: 6000000,
    targetRatio: 1.45,
    achievementRate: 92,
    rawMaterialAchievement: 94,
    subMaterialAchievement: 94,
    laborAchievement: 98,
    expenseAchievement: 97,
    rawMaterialVariance: -1000000,
    subMaterialVariance: -300000,
    laborVariance: -200000,
...
export const CostReportView: React.FC<Props> = () => {
  ...
  const handleTargetSave = useCallback(
    (month: string, target: Partial<CostTarget>) => {
      ...
      setSummaryData(prev =>
        prev.map(d => {
          if (d.month === month) {
            ...
            // 달성률 및 초과/절감 재계산
            const rawMaterialAchievement =
              newTarget.targetRawMaterial && newTarget.targetRawMaterial > 0
                ? Math.round((newTarget.targetRawMaterial / d.rawMaterialCost) * 100)
                : null;
            const subMaterialAchievement =
              newTarget.targetSubMaterial && newTarget.targetSubMaterial > 0
                ? Math.round((newTarget.targetSubMaterial / d.subMaterialCost) * 100)
                : null;
            const laborAchievement =
              newTarget.targetLabor && newTarget.targetLabor > 0
                ? Math.round((newTarget.targetLabor / d.laborCost) * 100)
                : null;
            const expenseAchievement =
              newTarget.targetExpense && newTarget.targetExpense > 0
                ? Math.round((newTarget.targetExpense / d.expenseAmount) * 100)
                : null;
  ...
  // 계정별 누적 초과금액 계산
  const cumulativeVarianceByAccount = (
    ['rawMaterial', 'subMaterial', 'labor', 'expense'] as AccountType[]
  ).map(account => {
    let cumVariance = 0;
    const data = summaryData.map(d => {
      const variance =
        account === 'rawMaterial'
          ? d.rawMaterialVariance
          : account === 'subMaterial'
            ? d.subMaterialVariance
            : account === 'labor'
              ? d.laborVariance
              : d.expenseVariance;
            ...
            <div className="flex-1">
              ...
              <details className="mt-2 text-xs text-amber-600">
                ...
                <ol className="mt-2 list-decimal ml-4 space-y-1">
                  ...
                  <li>
                    파일을 <code className="bg-amber-100 px-1 rounded">server/credentials/</code>{' '}
                    폴더에 저장
                  </li>
                  <li>
                    <code className="bg-amber-100 px-1 rounded">server/.env</code> 파일 수정:
              ...
              <table className="min-w-full text-sm">
                <thead className="bg-gray-50 dark:bg-gray-800">
                  <tr>
                    ...
                    {months.map(m => (
                      <th
                        key={m}
                        className="px-4 py-3 text-center font-medium text-gray-500 min-w-[120px]"
                      >
                        {formatMonth(m).replace('년 ', '/')}
                      </th>
                    ))}
                    <th className="px-4 py-3 text-center font-medium text-gray-500 bg-blue-50">
                      누적 초과
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                  ...
                  <tr className="bg-blue-50/50">
                    <td className="px-4 py-3 font-bold text-blue-700 border-r border-gray-200">
                      생산매출
                    </td>
                    {summaryData.map(d => (
                      <td key={d.month} className="px-4 py-3 text-center">
                        <div className="font-bold text-blue-600">
                          ₩{formatCurrency(d.salesAmount)}
                        </div>
                        {d.targetSales && (
                          <div className="text-xs text-gray-400">
                            목표: ₩{formatCurrency(d.targetSales)}
                          </div>
                        )}
                      </td>
                    ))}
                  ...
                  {/* 원재료액 */}
                  <tr>
                    <td className="px-4 py-3 font-medium text-orange-600 border-r border-gray-200">
                      원재료액
                    </td>
                    {summaryData.map(d => (
                      <td key={d.month} className="px-4 py-3 text-center">
                        ...
                        {d.targetRawMaterial && (
                          <div className="text-xs text-gray-400">
                            목표: ₩{formatCurrency(d.targetRawMaterial)}
                          </div>
                        )}
                        {d.rawMaterialVariance !== null && (
                          <div
                            className={`text-xs font-bold ${(d.rawMaterialVariance || 0) >= 0 ? 'text-green-600' : 'text-red-600'}`}
                          >
                            {(d.rawMaterialVariance || 0) >= 0 ? '▼' : '▲'} ₩
                            {formatCurrency(Math.abs(d.rawMaterialVariance || 0))}
                          </div>
                        )}
                      </td>
                    ))}
                    <td className="px-4 py-3 text-center bg-blue-50">
                      <span
                        className={`font-bold ${cumulativeVarianceByAccount[0].data.slice(-1)[0].cumVariance >= 0 ? 'text-green-600' : 'text-red-600'}`}
                      >
                        ₩
                        {formatCurrency(
                          Math.abs(cumulativeVarianceByAccount[0].data.slice(-1)[0].cumVariance)
                        )}
                      </span>
                    </td>
                  </tr>
                  {/* 부재료액 */}
                  <tr>
                    <td className="px-4 py-3 font-medium text-purple-600 border-r border-gray-200">
                      부재료액
                    </td>
                    {summaryData.map(d => (
                      <td key={d.month} className="px-4 py-3 text-center">
                        ...
                        {d.targetSubMaterial && (
                          <div className="text-xs text-gray-400">
                            목표: ₩{formatCurrency(d.targetSubMaterial)}
                          </div>
                        )}
                        {d.subMaterialVariance !== null && (
                          <div
                            className={`text-xs font-bold ${(d.subMaterialVariance || 0) >= 0 ? 'text-green-600' : 'text-red-600'}`}
                          >
                            {(d.subMaterialVariance || 0) >= 0 ? '▼' : '▲'} ₩
                            {formatCurrency(Math.abs(d.subMaterialVariance || 0))}
                          </div>
                        )}
                      </td>
                    ))}
                    <td className="px-4 py-3 text-center bg-blue-50">
                      <span
                        className={`font-bold ${cumulativeVarianceByAccount[1].data.slice(-1)[0].cumVariance >= 0 ? 'text-green-600' : 'text-red-600'}`}
                      >
                        ₩
                        {formatCurrency(
                          Math.abs(cumulativeVarianceByAccount[1].data.slice(-1)[0].cumVariance)
                        )}
                      </span>
                    </td>
                  </tr>
                  {/* 노무비액 */}
                  <tr>
                    <td className="px-4 py-3 font-medium text-teal-600 border-r border-gray-200">
                      노무비액
                    </td>
                    {summaryData.map(d => (
                      <td key={d.month} className="px-4 py-3 text-center">
                        ...
                        {d.targetLabor && (
                          <div className="text-xs text-gray-400">
                            목표: ₩{formatCurrency(d.targetLabor)}
                          </div>
                        )}
                        {d.laborVariance !== null && (
                          <div
                            className={`text-xs font-bold ${(d.laborVariance || 0) >= 0 ? 'text-green-600' : 'text-red-600'}`}
                          >
                            {(d.laborVariance || 0) >= 0 ? '▼' : '▲'} ₩
                            {formatCurrency(Math.abs(d.laborVariance || 0))}
                          </div>
                        )}
                      </td>
                    ))}
                    <td className="px-4 py-3 text-center bg-blue-50">
                      <span
                        className={`font-bold ${cumulativeVarianceByAccount[2].data.slice(-1)[0].cumVariance >= 0 ? 'text-green-600' : 'text-red-600'}`}
                      >
                        ₩
                        {formatCurrency(
                          Math.abs(cumulativeVarianceByAccount[2].data.slice(-1)[0].cumVariance)
                        )}
                      </span>
                    </td>
                  </tr>
                  {/* 경비액 */}
                  <tr>
                    <td className="px-4 py-3 font-medium text-red-600 border-r border-gray-200">
                      경비액
                    </td>
                    {summaryData.map(d => (
                      <td key={d.month} className="px-4 py-3 text-center">
                        ...
                        {d.targetExpense && (
                          <div className="text-xs text-gray-400">
                            목표: ₩{formatCurrency(d.targetExpense)}
                          </div>
                        )}
                        {d.expenseVariance !== null && (
                          <div
                            className={`text-xs font-bold ${(d.expenseVariance || 0) >= 0 ? 'text-green-600' : 'text-red-600'}`}
                          >
                            {(d.expenseVariance || 0) >= 0 ? '▼' : '▲'} ₩
                            {formatCurrency(Math.abs(d.expenseVariance || 0))}
                          </div>
                        )}
                      </td>
                    ))}
                    <td className="px-4 py-3 text-center bg-blue-50">
                      <span
                        className={`font-bold ${cumulativeVarianceByAccount[3].data.slice(-1)[0].cumVariance >= 0 ? 'text-green-600' : 'text-red-600'}`}
                      >
                        ₩
                        {formatCurrency(
                          Math.abs(cumulativeVarianceByAccount[3].data.slice(-1)[0].cumVariance)
                        )}
                      </span>
                    </td>
                  </tr>
            ...
            <div className="h-72">
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <YAxis
                    tick={{ fontSize: 11 }}
                    label={{ value: '비율', angle: -90, position: 'insideLeft', fontSize: 11 }}
                  />
            ...
            <div className="h-72">
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <YAxis
                    tick={{ fontSize: 11 }}
                    tickFormatter={v => `${v >= 0 ? '' : '-'}₩${formatCurrency(Math.abs(v))}`}
                  />
              ...
              <thead className="bg-gray-50 dark:bg-gray-800">
                <tr>
                  ...
                  {months.map(m => (
                    <th
                      key={m}
                      className="px-4 py-3 text-center font-medium text-gray-500 min-w-[140px]"
                    >
                      {formatMonth(m).replace('년 ', '/')}
                    </th>
                  ))}
        ...
        return (
          <td key={m} className="px-4 py-3 text-center">
            ...
            {isEditing ? (
              ...
              <div className="font-medium">₩{formatCurrency(parseFloat(values[m]) || 0)}</div>
            )}
          </td>
        );
      })}
